    ...
    <div class="tagblock panel-body">
        {if $sbdata.tags}
            {foreach from=$sbdata.tags item=tag}
                <a class="tag"{if $tag->size} style="font-size: {$tag->size}em;"{/if} href="{$WWWROOT}tags.php?tag={$tag->tag|urlencode|safe}" title="{str tag=nitems arg1=$tag->count}">{$tag->tag|str_shorten_text:20}{if $tag->owner}({$tag->owner}){/if}</a> &nbsp;
            {/foreach}
        {else}
