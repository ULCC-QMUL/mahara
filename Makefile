...
mergebase := $(shell git fetch gerrit >/dev/null 2>&1 && git merge-base HEAD gerrit/18.04_STABLE)
...
push: securitycheck minaccept
	...
	@if test -z "$(TAG)"; then \
		git push gerrit HEAD:refs/publish/18.04_STABLE; \
	else \
		git push gerrit HEAD:refs/publish/18.04_STABLE/$(TAG); \
	fi

security: minaccept
	@echo "Pushing the SECURITY change upstream..."
	@if test -z "$(TAG)"; then \
		git push gerrit HEAD:refs/drafts/18.04_STABLE; \
	else \
		git push gerrit HEAD:refs/drafts/18.04_STABLE/$(TAG); \
	fi
