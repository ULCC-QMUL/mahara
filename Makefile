...
ifdef simplesamlphp
	...
	@curl -sSL https://github.com/simplesamlphp/simplesamlphp/releases/download/v1.16.3/simplesamlphp-1.16.3.tar.gz | tar  --transform 's/simplesamlphp-[0-9]+\.[0-9]+\.[0-9]+/simplesamlphp/x1' -C auth/saml/extlib -xzf - # SimpleSAMLPHP release tarball already has all composer dependencies.
...
mergebase := $(shell git fetch gerrit >/dev/null 2>&1 && git merge-base HEAD gerrit/18.10_STABLE)
...
push: securitycheck minaccept
	...
	@if test -z "$(TAG)"; then \
		git push gerrit HEAD:refs/publish/18.10_STABLE; \
	else \
		git push gerrit HEAD:refs/publish/18.10_STABLE/$(TAG); \
	fi

security: minaccept
	@echo "Pushing the SECURITY change upstream..."
	@if test -z "$(TAG)"; then \
		git push gerrit HEAD:refs/drafts/18.10_STABLE; \
	else \
		git push gerrit HEAD:refs/drafts/18.10_STABLE/$(TAG); \
	fi
