                {foreach from=$views item=view}
                    <div class="list-group-item {if $view.submittedto} list-group-item-warning {/if}">
                        {* CUSTOM Catalyst - provide a different URL for QM Framework Dashboards. *}
                        {if $view.type == 'qmdashboard'}
                            <a href="{$WWWROOT}module/qmframework/dashboard.php?id={$view.id}" class="outer-link"><span class="sr-only">{$view.displaytitle}</span></a>
                        {elseif !$view.issitetemplate}
                            ...
                            <div class="col-md-3">
                                <div class="inner-link btn-action-list">
                                    <div class="btn-top-right btn-group btn-group-top">
                                        ...
                                        {/if}
                                        {* CUSTOM Catalyst - allow QM Framework Dashboard template deletion by site administrators *}
                                        {if !$view.submittedto && $view.removable && (!$view.locked || $editlocked)
                                            || ($view.type == 'qmdashboard' && $view.institution != null && $USER->get('admin'))}
